...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (scrollDown) {
    scrollDown.addEventListener("click", () => {
      ...
      if (whySection) {
        const top =
          whySection.getBoundingClientRect().top + window.scrollY - 76;
        window.scrollTo({ top, behavior: "smooth" });
      }
    });
  }
  ...
  document.querySelectorAll(".prog-card").forEach((card) => {
    ...
    card.addEventListener("mouseleave", () => {
      ...
      card.style.transition =
        "transform 0.4s ease, box-shadow 0.35s, border-color 0.35s";
    });
  });
  ...
  const facObserver = new IntersectionObserver(
    (entries) => {
  ...
  console.log(
    "%c🧠 Brain Stormers Academic & Admission Care",
    "color:#E8820C;font-size:16px;font-weight:bold;font-family:serif;",
  );
  console.log(
    "%c✨ Website crafted with precision & care.",
    "color:#1A2B52;font-size:12px;",
  );
});
